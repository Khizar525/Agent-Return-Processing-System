...
# The SDK's __init__.py likely overwrote __path__ with only its own
# directory.  Ensure the local agents/ directory is still findable so
# that submodules like triage_orchestrator can be imported.
if _our_dir not in __path__:
    __path__.append(_our_dir)
